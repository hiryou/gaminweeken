...
plus the supporting scripts under `scripts/`) to handle device initialization, host-side APK management, and
...
```
If you want the command arguments to follow the tracked device settings automatically, read them from
...
2. Configure ES-DE frontend routing using the dedicated mapping reference:
   [es-de-mapping.md](./es-de-mapping.md)
3. Restore your exported `Key Mapper` backup ZIP if you use controller-only OS shortcuts such as:
...
4. If you want to use RetroArch-backed systems such as `nes`, install the matching RetroArch cores:
```bash
./setup_retroarch.sh
```
...
5. Check whether any RetroArch BIOS / firmware files are still missing for `ps1` and `dreamcast`:
```bash
